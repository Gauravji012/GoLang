...
Init modules: go mod init student-api && go mod tidy.

Edit local.yaml for DB path/server addr.
​
...
text
student-api/
├── main.go      # Entry: config → DB → router → server[file:21]
├── conf.go      # YAML config loader[file:17]
├── local.yaml   # Config file[file:18]
├── student.go   # Models + handlers[file:19]
├── types.go     # Shared types[file:20]
├── response.go  # JSON helpers[file:22]
├── sqlite.go    # DB init/queries[file:23]
└── storage.go   # Interface[file:24]
How It Works
